KISSY.use('form/uploader/queue/base', function (S, Queue) {
    describe('Queue', function () {
        ...
        it('成功向队列添加一个文件', function () {
            var file;
            queue.add(testFile, function (index, fileData) {
                file = fileData;
            });
            waitsFor(function () {
                return S.isObject(file);
            }, 'file never a object', 1000);
            runs(function () {
        ...
        it('传递文件数组索引值，成功删除一个文件', function () {
            var file;
            queue.remove(0, function (index, fileData) {
                file = fileData;
            });
            waitsFor(function () {
                return S.isObject(file);
        ...
        it('批量向队列增加文件', function () {
            var files = [], isAddEnd = false;
            for (var i = 0; i < 3; i++) {
                files.push(S.merge(testFile, {name:'file' - i}));
            }
